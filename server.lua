...
RegisterNetEvent('qb-aircraftshop:server:addPlayer', function(citizenid, gameTime)
    financetimer[citizenid] = gameTime
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('qb-aircraftshop:server:removePlayer', function(citizenid)
...
QBCore.Functions.CreateCallback('qb-aircraftshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('qb-aircraftshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('qb-aircraftshop:client:swapVehicle', -1, data)
    Wait(1500) -- let new car spawn
    TriggerClientEvent('qb-aircraftshop:client:homeMenu', src) -- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('qb-aircraftshop:server:customTestDrive', function(data)
    local src = source
    ...
    TriggerClientEvent('qb-aircraftshop:client:customTestDrive', testDrivePlayer.PlayerData.source, data)
end)

-- Make a finance payment
RegisterNetEvent('qb-aircraftshop:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-aircraftshop:server:financePaymentFull', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
...
RegisterNetEvent('qb-aircraftshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    if cash > vehiclePrice then
        ...
        TriggerClientEvent('qb-aircraftshop:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('qb-aircraftshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
...
RegisterNetEvent('qb-aircraftshop:server:sellShowroomVehicle', function(data)
    local src = source
    local PlayerPed = GetPlayerPed(src)
    local pCoords = GetEntityCoords(PlayerPed)
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if cash >= vehiclePrice then
        ...
        TriggerClientEvent('qb-aircraftshop:client:buyShowroomVehicle', targetPlayer.PlayerData.source, vehicle, plate)
...
RegisterNetEvent('qb-aircraftshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
    local PlayerPed = GetPlayerPed(src)
    local pCoords = GetEntityCoords(PlayerPed)
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-aircraftshop:server:checkFinance', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
